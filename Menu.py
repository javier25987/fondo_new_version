...
import datetime
import os
import funciones.general as f_g
    ...
    with c_1:
        ...
        if st.button('📤 Guardar en GitHub'):
            with st.status("Guardando cambios...", expanded=True) as status:
                ...
                f_g.ejecutar_comando_git(["git", "add", "."])
                ...
                f_g.ejecutar_comando_git(["git", "commit", "-m", mensaje_de_comit])
                with open('ajustes.json', 'w') as f:
                    json.dump(ajustes, f)
                    f.close()

                st.write('Guardando en GitHub...')
                f_g.ejecutar_comando_git(
                    ["git", "push"]
                )
                status.update(
...
with tab_2:
    ...
    if st.button("Cargar Multas"):
        f_g.arreglar_todos_los_asuntos()
        st.success(
